...
class QuesinfoerSpider(scrapy.Spider):
    ...
    #    inspect_response(response,self)
       ...
       # item['visitsCount'] = response.xpath('//*[@id="zh-single-question-page"]/meta[@itemprop="visitsCount"]/@content').extract()[0]    #right
        ...
        #item['questionDetailSummary'] = response.xpath('//div[@id="zh-question-detail"]/div[@class= summary]/text()').extract()[0]

        item['dataResourceId'] = response.xpath('//div[@id="zh-question-detail"]/@data-resourceid').extract()[0]    #right

        #item['zmEditableContent'] = response.xpath('//*[@id="zh-question-detail"]/div').extract()
        #item['questionContentHidden'] = response.xpath('//*[@id="zh-question-detail"]/textarea').extract()
        #item['questionSummary'] = response.xpath('//*[@id="zh-question-detail"]/div').extract()
